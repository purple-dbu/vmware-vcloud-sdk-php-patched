<?php
class VMware_VCloud_API_QueryResultNetworkPoolRecordType extends VMware_VCloud_API_QueryResultRecordType {
    protected $isBusy = null;
    protected $networkPoolType = null;
    protected $name = null;
    ...
    * @param int $networkPoolType - [optional] an attribute, 
    * @param string $name - [optional] an attribute, 
    */
    public function __construct($href=null, $type=null, $id=null, $Link=null, $Metadata=null, $isBusy=null, $networkPoolType=null, $name=null) {
        parent::__construct($href, $type, $id, $Link, $Metadata);
        $this->isBusy = $isBusy;
        $this->networkPoolType = $networkPoolType;
        $this->name = $name;
        $this->tagName = 'NetworkPoolRecord';
    ...
    public function get_networkPoolType(){
        return $this->networkPoolType;
    }
    public function set_networkPoolType($networkPoolType) {
        $this->networkPoolType = $networkPoolType;
    }
    ...
    protected function exportAttributes($out, $level, $name, $namespace, $rootNS) {
        ...
        if (!is_null($this->networkPoolType)) {
            $ns = VMware_VCloud_API_Helper::getAttributeNamespaceTag($this->namespace, 'networkPoolType', self::$defaultNS, $namespace, $rootNS);
            $out = VMware_VCloud_API_Helper::addString($out, ' ' . $ns . 'networkPoolType=' . VMware_VCloud_API_Helper::quote_attrib(VMware_VCloud_API_Helper::format_integer($this->networkPoolType, $input_name='networkPoolType')));
        }
    ...
    protected function buildAttributes($node, $namespaces='') {
        ...
        $ndnetworkPoolType = $attrs->getNamedItem('networkPoolType');
        if (!is_null($ndnetworkPoolType)) {
            $this->networkPoolType = sprintf('%d', $ndnetworkPoolType->value);
            if (isset($ndnetworkPoolType->prefix)) {
                $this->namespace['networkPoolType'] = $ndnetworkPoolType->prefix;
                $nsUri = $ndnetworkPoolType->lookupNamespaceURI($ndnetworkPoolType->prefix);
            }
            $node->removeAttributeNS($nsUri, 'networkPoolType');
        } else {
            $this->networkPoolType = null;
        }
