<?php
class VMware_VCloud_API_NetworkConnectionSectionType extends VMware_VCloud_API_OVF_Section_Type {
    protected $href = null;
    protected $type = null;
    protected $PrimaryNetworkConnectionIndex = null;
    protected $NetworkConnection = array();
    ...
    * @param int $PrimaryNetworkConnectionIndex - [optional] 
    * @param array $NetworkConnection - [optional] an array of VMware_VCloud_API_NetworkConnectionType objects
    * @param array $Link - [optional] an array of VMware_VCloud_API_LinkType objects
    * @param array $Any - [optional] an array of any type of objects
    */
    public function __construct($required=null, $Info=null, $href=null, $type=null, $PrimaryNetworkConnectionIndex=null, $NetworkConnection=null, $Link=null, $Any=null) {
        parent::__construct($required, $Info);
        $this->href = $href;
        $this->type = $type;
        $this->PrimaryNetworkConnectionIndex = $PrimaryNetworkConnectionIndex;
        if (!is_null($NetworkConnection)) {
        ...
        $this->tagName = 'NetworkConnectionSection';
        $this->namespacedef = ' xmlns:vcloud="http://www.vmware.com/vcloud/v1.5" xmlns:ns12="http://www.vmware.com/vcloud/v1.5" xmlns:ovf="http://schemas.dmtf.org/ovf/envelope/1" xmlns:ovfenv="http://schemas.dmtf.org/ovf/environment/1" xmlns:vmext="http://www.vmware.com/vcloud/extension/v1.5" xmlns:cim="http://schemas.dmtf.org/wbem/wscim/1/common" xmlns:rasd="http://schemas.dmtf.org/wbem/wscim/1/cim-schema/2/CIM_ResourceAllocationSettingData" xmlns:vssd="http://schemas.dmtf.org/wbem/wscim/1/cim-schema/2/CIM_VirtualSystemSettingData" xmlns:vmw="http://www.vmware.com/schema/ovf" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"';
    }
    public function getPrimaryNetworkConnectionIndex() {
        return $this->PrimaryNetworkConnectionIndex;
    }
    public function setPrimaryNetworkConnectionIndex($PrimaryNetworkConnectionIndex) { 
        $this->PrimaryNetworkConnectionIndex = $PrimaryNetworkConnectionIndex;
    }
    public function getNetworkConnection() {
        return $this->NetworkConnection;
    }
    public function setNetworkConnection($NetworkConnection) { 
        $this->NetworkConnection = $NetworkConnection;
    }
    public function addNetworkConnection($value) { array_push($this->NetworkConnection, $value); }
    ...
    protected function exportChildren($out, $level, $name, $namespace, $rootNS) {
        ...
        if (!is_null($this->PrimaryNetworkConnectionIndex)) {
            $out = VMware_VCloud_API_Helper::showIndent($out, $level);
            $ns = VMware_VCloud_API_Helper::getNamespaceTag($this->namespace, 'PrimaryNetworkConnectionIndex', self::$defaultNS, $namespace, $rootNS);
            $out = VMware_VCloud_API_Helper::addString($out, "<" . $ns . "PrimaryNetworkConnectionIndex>" . VMware_VCloud_API_Helper::format_integer($this->PrimaryNetworkConnectionIndex, $input_name='PrimaryNetworkConnectionIndex') . "</" . $ns . "PrimaryNetworkConnectionIndex>\n");
        }
        if (isset($this->NetworkConnection)) {
            foreach ($this->NetworkConnection as $NetworkConnection) {
                $out = $NetworkConnection->export('NetworkConnection', $out, $level, '', $namespace, $rootNS);
            }
        }
        if (isset($this->Link)) {
    ...
    protected function hasContent() {
        if (
            !is_null($this->PrimaryNetworkConnectionIndex) ||
            !empty($this->NetworkConnection) ||
            !empty($this->Link) ||
            !empty($this->Any) ||
            parent::hasContent()
            ) {
    ...
    protected function buildChildren($child, $nodeName, $namespace='') {
        if ($child->nodeType == XML_ELEMENT_NODE && $nodeName == 'PrimaryNetworkConnectionIndex') {
            $sval = $child->nodeValue;
            $this->PrimaryNetworkConnectionIndex = $sval;
            if (!empty($namespace)) {
                $this->namespace['PrimaryNetworkConnectionIndex'] = $namespace;
            }
        }
        elseif ($child->nodeType == XML_ELEMENT_NODE && $nodeName == 'NetworkConnection') {
            $obj = new VMware_VCloud_API_NetworkConnectionType();
            $obj->build($child);
            $obj->set_tagName('NetworkConnection');
            array_push($this->NetworkConnection, $obj);
            if (!empty($namespace)) {
                $this->namespace['NetworkConnection'] = $namespace;
            }
        }
