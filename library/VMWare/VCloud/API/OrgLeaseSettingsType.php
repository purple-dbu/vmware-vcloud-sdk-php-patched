<?php
class VMware_VCloud_API_OrgLeaseSettingsType extends VMware_VCloud_API_ResourceType {
    ...
    * @param int $DeploymentLeaseSeconds - [optional] 
    * @param int $StorageLeaseSeconds - [optional] 
    */
    public function __construct($VCloudExtension=null, $href=null, $type=null, $Link=null, $DeleteOnStorageLeaseExpiration=null, $DeploymentLeaseSeconds=null, $StorageLeaseSeconds=null) {
        ...
        $this->tagName = 'VAppLeaseSettings';
    ...
    public function getDeploymentLeaseSeconds() {
        return $this->DeploymentLeaseSeconds;
    }
    public function setDeploymentLeaseSeconds($DeploymentLeaseSeconds) { 
        $this->DeploymentLeaseSeconds = $DeploymentLeaseSeconds;
    }
    ...
    protected function exportChildren($out, $level, $name, $namespace, $rootNS) {
        ...
        if (!is_null($this->DeploymentLeaseSeconds)) {
            $out = VMware_VCloud_API_Helper::showIndent($out, $level);
            $ns = VMware_VCloud_API_Helper::getNamespaceTag($this->namespace, 'DeploymentLeaseSeconds', self::$defaultNS, $namespace, $rootNS);
            $out = VMware_VCloud_API_Helper::addString($out, "<" . $ns . "DeploymentLeaseSeconds>" . VMware_VCloud_API_Helper::format_integer($this->DeploymentLeaseSeconds, $input_name='DeploymentLeaseSeconds') . "</" . $ns . "DeploymentLeaseSeconds>\n");
        }
    ...
    protected function hasContent() {
        if (
            ...
            !is_null($this->DeploymentLeaseSeconds) ||
    ...
    protected function buildChildren($child, $nodeName, $namespace='') {
        ...
        elseif ($child->nodeType == XML_ELEMENT_NODE && $nodeName == 'DeploymentLeaseSeconds') {
            $sval = $child->nodeValue;
            $this->DeploymentLeaseSeconds = $sval;
            if (!empty($namespace)) {
                $this->namespace['DeploymentLeaseSeconds'] = $namespace;
            }
        }
