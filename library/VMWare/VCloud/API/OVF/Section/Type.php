<?php
class VMware_VCloud_API_OVF_Section_Type {
    ...
    */
    public function __construct($required=null, $Info=null) {
    ...
    protected function exportAttributes($out, $level, $name, $namespace, $rootNS) {
        ...
        if (!is_null($this->required)) {
            ...
            $out = VMware_VCloud_API_Helper::addString($out, ' ' . $ns . 'required=' . VMware_VCloud_API_Helper::quote_attrib(VMware_VCloud_API_Helper::format_string(mb_convert_encoding($this->required, VMware_VCloud_API_Helper::$ExternalEncoding, "auto"), $input_name='required')));
        }
    ...
    protected function exportChildren($out, $level, $name, $namespace, $rootNS) {
        ...
        if (!is_null($this->Info)) {
            $out = $this->Info->export('Info', $out, $level, '', $namespace, $rootNS);
        }
        return $out;
    }
    ...
    protected function buildAttributes($node, $namespaces='') {
        ...
        if (!is_null($ndrequired)) {
            $this->required = $ndrequired->value;
    ...
    protected function buildChildren($child, $nodeName, $namespace='') {
        if ($child->nodeType == XML_ELEMENT_NODE && $nodeName == 'Info') {
            $obj = new VMware_VCloud_API_OVF_Msg_Type();
            $obj->build($child);
            $obj->set_tagName('Info');
            $this->setInfo($obj);
            if (!empty($namespace)) {
