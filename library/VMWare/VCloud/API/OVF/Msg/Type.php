<?php
class VMware_VCloud_API_OVF_Msg_Type {
   ...
   /**
    * @param string $msgid - [optional] an attribute, default ''
    * @param string $valueOf - [optional]
    */
    public function __construct($msgid='', $valueOf='') {
        $this->msgid = $msgid;
    ...
    protected function exportAttributes($out, $level, $name, $namespace, $rootNS) {
        ...
        if (!is_null($this->msgid)) {
            $ns = VMware_VCloud_API_Helper::getAttributeNamespaceTag($this->namespace, 'msgid', self::$defaultNS, $namespace, $rootNS);
            $out = VMware_VCloud_API_Helper::addString($out, ' ' . $ns . 'msgid=' . VMware_VCloud_API_Helper::quote_attrib(VMware_VCloud_API_Helper::format_string(mb_convert_encoding($this->msgid, VMware_VCloud_API_Helper::$ExternalEncoding, "auto"), $input_name='msgid')));
        }
    ...
    protected function buildAttributes($node, $namespaces='') {
        ...
        $ndmsgid = $attrs->getNamedItem('msgid');
        if (!is_null($ndmsgid)) {
            $this->msgid = $ndmsgid->value;
            if (isset($ndmsgid->prefix)) {
                $this->namespace['msgid'] = $ndmsgid->prefix;
                $nsUri = $ndmsgid->lookupNamespaceURI($ndmsgid->prefix);
            }
            $node->removeAttributeNS($nsUri, 'msgid');
        } else {
            $this->msgid = null;
        }
        $this->anyAttributes = array();
